...
class Propeller:
    ...
    def gen_geom(self, arg_key: Optional[str]):
        ...
        self.gen_geom_parts(arg_key)
        self.gen_geom_prop(arg_key)
        self.gen_geom_boundaries(arg_key)

    def gen_geom_parts(self, arg_key: Optional[str]):
        self.blade = Blade(self.blade_cnf, self.geompy, self.ref_pnt_blades, self.ref_axis)
        self.blade.gen_blade()
        if self.blade.blade is not None:
    ...
    def gen_geom_boundaries(self, arg_key: Optional[str]):
    ...
    def export_geo(self, arg_dir: str):
        ...
        self.geompy.ExportSTEP(self.propeller, tmp_step_file, GEOM.LU_METER)
        tmp_stl_file = os.path.join(arg_dir, "propeller.stl")
        self.geompy.ExportSTL(self.propeller, tmp_stl_file, False, 0.001, True)
